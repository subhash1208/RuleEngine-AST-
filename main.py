...
def parse_rule_string(rule_string):
    ...
    def parse_expression():
        ...
        def flatten_expr(expr):
            ...
            for item in expr:
                if isinstance(item, list):
                    flat_expr.extend(flatten_expr(item))
                else:
        ...
        def build_tree(expr):
            ...
            # Flatten expr before joining
            return Node('operand', ' '.join(flatten_expr(expr)))
...
@app.route('/')
def home():
    return "Welcome to the Rule Engine API! Use the available endpoints to create, combine, evaluate, and modify rules."
